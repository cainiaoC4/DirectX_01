...
protected:
    struct STATE
    {
        ...
        LPDXUTCALLBACKCMDLINEARG                m_CommandLineArgFunc;           // command line arguments callback
        ...
        // Intel:
        void* m_CommandLineArgFuncUserContext;           // user context for command line argument callback
...
public:
    ...
    // Intel
    GET_SET_ACCESSOR( LPDXUTCALLBACKCMDLINEARG, CommandLineArgFunc );
    ...
    // Intel:
    GET_SET_ACCESSOR( void*, CommandLineArgFuncUserContext );
...
// Intel:
void WINAPI DXUTSetCallbackCommandLineArg( LPDXUTCALLBACKCMDLINEARG pCallback, void* pUserContext )                            { GetDXUTState().SetCommandLineArgFunc( pCallback );  GetDXUTState().SetCommandLineArgFuncUserContext( pUserContext ); }
                          ...
                          bool bIgnoreFirstCommand  )
{
    ...
    // Intel: Grab pointer to command line argument handler callback if present
    LPDXUTCALLBACKCMDLINEARG pCommandLineArgFunc = GetDXUTState().GetCommandLineArgFunc();
    void *pCommandLineArgFuncUserContext = GetDXUTState().GetCommandLineArgFuncUserContext();
    ...
    for( int iArg = iArgStart; iArg < nNumArgs; iArg++ )
    {
        ...
        // Unrecognized flag
        // Intel: Also separate out param portion
        ...
        *strSpace = 0;
        ...
        // Intel: Call user command-line handler if present
        if (!pCommandLineArgFunc || !pCommandLineArgFunc(strFlag, strParam, pCommandLineArgFuncUserContext))
        {
